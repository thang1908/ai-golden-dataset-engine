"""Thread-safe, terminal-only counters for one G1 prediction case."""
...
class CaseMetrics:
    ...
    def record(self, context: CallContext, *, http_attempt: int, outcome: str, input_token: int | None, output_token: int | None) -> None:
        with self._lock:
            ...
            print(f"[g1] sample={context.sample_id} stage={context.stage} request={self._data['num_request']} outcome={outcome} input_token={input_token} output_token={output_token}", flush=True)
