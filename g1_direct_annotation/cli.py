...
DEFAULT_OUTPUT_DIR = REPOSITORY_ROOT / "output" / "g1"


def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description="G1: generate caption + 21 attributes for the supplied test dataset"
    )
    result.add_argument(
        "--test-dir",
        type=Path,
    ...
    result.add_argument(
        ...
        help="Output folder (default: output/g1)",
    )
    ...
    result.add_argument("--model", dest="G1_MODEL")
    ...
    try:
        ...
        with VllmClient(settings) as client:
            def annotate(image: bytes):
                ...
                return {**annotation, "caption_vi": client.translate_caption(annotation["caption"])}
            rows = generate_dataset(samples, annotate, model=settings.model)
        ...
        print(f"G1 generated {succeeded}/{len(rows)} predictions: {target}")
        return 0 if succeeded == len(rows) else 1
    except (G1Error, OSError, ValueError) as exc:
        print(f"G1 error: {exc}", file=sys.stderr)
        return 1
