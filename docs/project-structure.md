...
### Per-case counter ownership

```text
g1_direct_annotation/telemetry.py          # [E] in-memory counter owned by G1
g2_facts_then_caption/telemetry.py         # [E] in-memory counter owned by G2
g3_consensus_annotation/telemetry.py       # [E] in-memory counter owned by G3
g4_critic_verifier/telemetry.py            # [E] in-memory counter owned by G4
g5_qa_refinement/telemetry.py              # [E] in-memory counter owned by G5
output/g{1,2,3,4,5}/predictions.jsonl      # [G] final row contains token/request counters
```
