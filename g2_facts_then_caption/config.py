"""Read shared repository VLLM settings; G2 remains Medium by default."""
...
class Settings:
    ...
    def __post_init__(self) -> None:
        ...
        if self.timeout_seconds <= 0 or self.max_tokens < 64 or self.max_retries < 1 or self.max_requests_per_minute < 1:
            raise ConfigurationError("G2 timeout, max tokens, retries or request limit are invalid")
    ...
    *, dotenv_path: Path = DEFAULT_DOTENV_PATH, overrides: dict[str, object] | None = None
    ...
    try:
        return Settings(
            ...
            # Shared VLLM_MODEL may target Large, so G2 never reads it.
            model=value("G2_MODEL", default=DEFAULT_MODEL),
        ...
        raise ConfigurationError("G2 numeric configuration is invalid") from exc
